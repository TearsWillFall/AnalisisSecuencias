...
def plot(go_list, kegg_list, min_support, name):
    custom_style = Style(
        opacity='0.8',
        opacity_hover='0.5',
        title_font_size=36,
    ...
    multi_pie.title = "Functional annotations"
    ...
    html_file.write(f"<!doctype html><html><head><meta charset=\"utf-8\"><title>{name}\
        </title></head><body style=\"background-color: #f9f9f9;\"><div style=\"display: flex;\">\
        <object type=\"image/svg+xml\"data=\"{plot_file_name}\" height=\"800\"></object><div>\
        <h2>GO:</h2><ul>")
    ...
    html_file.write(f"</ul></div><div><h2>KEGG:</h2><ul>")

    for kegg_item in kegg:
        html_file.write(f"<li><strong>{go_item['value']}x</strong>\
            <a target=\"_blank\" href=\"{kegg_item['xlink']['href']}\">{kegg_item['label']}</li>")

    html_file.write(f"</ul></div></div>")
